...
func (rw *Worker) RunTxTaskNoLock(txTask *state.TxTask, isMining, skipPostEvaluation bool) {
	...
	case txTask.Final:
		...
		if isMining {
			...
			_, err = rw.engine.Finalize(cc, types.CopyHeader(header), ibs, txTask.Txs, txTask.Uncles, txTask.BlockReceipts, txTask.Withdrawals, rw.chain, syscall, skipPostEvaluation, nil, txTask.TxIndex, rw.logger)
		}
		if err != nil {
			txTask.Error = err
		} else {
	...
	case txTask.SystemTxIndex > 0:
		systemCall := func(ibs *state.IntraBlockState) ([]byte, bool, error) {
			...
			if err != nil {
				...
				txTask.Failed = false
				txTask.GasUsed = msg.Gas() - leftOverGas
		...
		_, err := rw.engine.Finalize(rw.chainConfig, types.CopyHeader(header), ibs, txTask.Txs, txTask.Uncles, txTask.BlockReceipts, txTask.Withdrawals, rw.chain, nil, skipPostEvaluation, systemCall, txTask.TxIndex, rw.logger)
		if err != nil {
			txTask.Error = err
		}
	default:
